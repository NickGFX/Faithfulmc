...
public class GlowstoneMountainManager implements Runnable {
    private static final long DEFAULTTIME = TimeUnit.HOURS.toMillis(1);

    private final HCF hcf;
    private final File schematicfile = new File("plugins/WorldEdit/schematics/glowstone.schematic");
    ...
    public GlowstoneMountainManager(HCF hcf) {
        this.hcf = hcf;
        config = new Config(hcf, "glowstonemountain");
    ...
    public boolean updateSelection() {
        if (!schematicfile.exists()) {
            System.out.println("No glowstone mountain schematic defined");
        } else if (cuboid == null) {
    ...
    public void setBlocks() {
        if (cuboidclipboard == null || cuboid == null || cuboid.getWorld() == null) {
            System.out.println("No glowstone mountain cuboid defined");
        } else {
            try {
                ...
                if (w == null) {
                    System.out.println("No glowstone mountain schematic world defined");
                } else {
                    ...
                    for (int x = 0; x < size.getBlockX(); ++x) {
                        for (int y = 0; y < size.getBlockY(); ++y) {
                            for (int z = 0; z < size.getBlockZ(); ++z) {
                                ...
                                if (block != null && block.getType() == Material.GLOWSTONE.getId()) {
    ...
    public void run() {
        if (cuboid != null) {
            ...
            if (diff > time) {
                setBlocks();
                lasttime = now;
                Bukkit.broadcastMessage(ChatColor.YELLOW + "The " + ChatColor.GOLD + ChatColor.BOLD + "Glowstone Mountain" + ChatColor.YELLOW + " has reset");
                messages.clear();
            } else {
                for (int time : msgs) {
                    long duration = this.time - TimeUnit.MINUTES.toMillis(time);
                    if (diff > duration && messages.add(time)) {
                        Bukkit.broadcastMessage(ChatColor.YELLOW + "The " + ChatColor.GOLD + ChatColor.BOLD + "Glowstone Mountain" + ChatColor.YELLOW + " is resetting in " + ChatColor.GOLD + DurationFormatUtils.formatDurationWords(TimeUnit.MINUTES.toMillis(time), true, true));
                        break;
                    }
                }
            }
